...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['cancel_all'])) {
        ...
        $delete_query = "DELETE FROM bookings WHERE email='$email'";
        if (mysqli_query($conn, $delete_query)) {
            echo "<script>alert('All bookings canceled successfully!'); window.location.href='user_profile.php';</script>";
        } else {
            echo "Error canceling bookings: " . mysqli_error($conn);
        }
        ...
        $delete_query = "DELETE FROM bookings WHERE email='$email' AND reservation_date='$cancel_date'";
        if (mysqli_query($conn, $delete_query)) {
        ...
        echo "<script>alert('Please select a date to cancel.'); window.location.href='cancel_booking.php?email=$email';</script>";
    }
...
<body>
    <div class="container">
        ...
        <form method="POST">
            ...
            <select name="cancel_date" id="cancel_date">
                ...
                <?php foreach ($bookings as $booking) : ?>
                    <option value="<?= htmlspecialchars($booking['reservation_date']) ?>">
                        <?= htmlspecialchars($booking['reservation_date']) ?>
                    </option>
                <?php endforeach; ?>
            </select>
        ...
        <form method="POST">
            <input type="hidden" name="email" value="<?= htmlspecialchars($email) ?>">
            <button type="submit" name="cancel_all" style="background-color: red; color: white;" onclick="return confirm('Are you sure you want to cancel all bookings?');">
                Cancel All Bookings
            </button>
        </form>
    </div>
